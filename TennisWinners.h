...
public:
    ...
    TennisWinners(std::string tour, std::string level, std::string win, float age, std::string score){
        tournoment = tour;
        this->level = level;
        winner = win;
        this->age = age;
...
void getTennisWinnersFromFile(std::string filename, std::vector<TennisWinners> &tWinners){
    ...
    std::string tournoment = "", level = "", winner = "", score = "", notUsed = "";
    float age = 0.0;
    char comma = ',';
    ...
    while(fin && fin.peek() != EOF){
        ...
        tWinners.push_back(TennisWinners(tournoment, level, winner, age, score));
    }
    fin.close();
}
